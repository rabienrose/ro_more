...
func get_min_score_node(node_list):
    ...
    for i in range(node_list.size()):
        if min_score==-1 or min_score>node_list[i]["score"]:
            min_score=node_list[i]["score"]
            min_node=i
    ...
    while true:
        ...
        for i in range(8):
            ...
            if next_c_ind in done_nodes:
                ...
                new_node["score"]=new_node["dist"]+new_node["cost"]
                pending_nodes[next_c_ind]=map.cells[t_id]
